...
% -- compare matrices entry-wise (Phase 1: placeholder tables -> exact match)
...
assert(relF < 1e-12, 'galerkin matrix differs from ggq matrix');
...
% L2-projection sanity check: aux samples of a polynomial of degree <= k-1
% must reproduce its values at the disc nodes via ipw.
deg = chnkr.k - 1;
c = randn(deg+1,1);
ts_a = auxquads.ts_aux; ts_d = auxquads.ts_disc;
fa = polyval(flipud(c),ts_a);
fd = polyval(flipud(c),ts_d);
err_proj = norm(auxquads.ipw*fa - fd,inf);
fprintf('ipw polynomial-recovery error = %5.2e\n',err_proj);
assert(err_proj < 1e-10, 'ipw fails to reproduce polynomial of degree <= k-1');
